...
def gradient_check(x, theta, epsilon = 1e-7):
    ...
    ### START CODE HERE ### (approx. 1 line)
    numerator = np.sqrt(np.power((gradapprox - grad),2))                               # Step 1'
    denominator = np.sqrt(gradapprox*gradapprox) +np.sqrt(grad*grad)                             # Step 2'
...
def gradient_check_n(parameters, gradients, X, Y, epsilon = 1e-7):
    ...
    for i in range(num_parameters):
        ...
        ### START CODE HERE ### (approx. 1 line)
        gradapprox[i] = (J_plus[i] - J_minus[i])*(1/(2*epsilon))
        ### END CODE HERE ###

    # Compare gradapprox to backward propagation gradients by computing difference.
    ### START CODE HERE ### (approx. 1 line)
    numerator = np.sqrt(np.sum(np.square(gradapprox - grad)))                                          # Step 1'
    denominator = np.sqrt(np.sum(np.square(gradapprox))) + np.sqrt(np.sum(np.square(grad)))            # Step 2'
    difference = numerator/denominator                                           # Step 3'
    ### END CODE HERE ###
